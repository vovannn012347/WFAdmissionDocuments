...
{
    public class KeyedRegexTextTemplate : ITemplateKeyedRegexText
    {
        [JsonIgnore]
        ...
        {
            return new TextRegexTemplateElement();
        }

        public void PopulateFromElement(ITemplateElement element)
        {
            Parent = element;
            var el = (element as TextRegexTemplateElement);
            ...
            this.RegexPattern = el.RegexPattern;
            this.CaptureGroup = el.CaptureGroup;
        ...
        public void PopulateToElement(ITemplateElement element)
        {
            ...
            el.RegexPattern = this.RegexPattern;
            el.CaptureGroup = this.CaptureGroup;
        ...
        public void PopulatePdfCanvas(Canvas canvas, SizeF size, ISettingsData settings, Dictionary<string, iText.Kernel.Font.PdfFont> fontCollection, List<ITemplateData> elementList, Dictionary<string, string> data = null)
        {
            ...
            if (match.Success)
            {
                if(CaptureGroup < match.Groups.Count) {
                    text = match.Groups[CaptureGroup].Value;
                }
            }
